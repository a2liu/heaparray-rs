//! Contains definition for `TpAcArray`, which is a thin pointer to an reference-counted
//! array, and the implementation for `heaparray::naive_rc::RcArray`.
pub use crate::naive_rc::prelude::*;

type PtrType<'a, E, L> = ThinPtrArray<'a, E, RcStruct<L>>;
type DataType<'a, E, L> = ManuallyDrop<PtrType<'a, E, L>>;

/// A thin-pointer to a reference-counted array. Has the same API as the `HeapArray`,
...
///
/// ```rust
/// use heaparray::naive_rc::thin_rc_array::*;
/// let array_ref = TpAcArray::<i32, (&str, &str)>::new_labelled(("hello", "world"), 10, |_,_| 0);
...
/// }
/// ```
pub struct TpAcArray<'a, E, L = ()> {
    data: DataType<'a, E, L>,
}

impl<'a, E> TpAcArray<'a, E> {
...
impl<'a, E, L> Index<usize> for TpAcArray<'a, E, L> {
    type Output = E;
    #[inline]
    fn index(&self, idx: usize) -> &E {
        assert!(!self.is_null(), "Null dereference of heaparray::TpAcArray");
        &self.data[idx]
    }
}

impl<'a, E, L> IndexMut<usize> for TpAcArray<'a, E, L> {
    #[inline]
...
impl<'a, E, L> Clone for TpAcArray<'a, E, L> {
    #[inline]
    fn clone(&self) -> Self {
        if self.is_null() {
            Self::null_ref()
...
impl<'a, E, L> Drop for TpAcArray<'a, E, L> {
    fn drop(&mut self) {
        self.to_null();
    }
}

impl<'a, E, L> Container<(usize, E)> for TpAcArray<'a, E, L> {
    #[inline]
...
impl<'a, E, L> CopyMap<'a, usize, E> for TpAcArray<'a, E, L>
where
    E: 'a,
{
    #[inline]
    ...
    fn insert(&mut self, key: usize, value: E) -> Option<E> {
        assert!(!self.is_null(), "Null dereference of heaparray::TpAcArray");
        if self.is_null() {
            panic!("Null dereference of heaparray::TpAcArray");
...
impl<'a, E, L> Array<'a, E> for TpAcArray<'a, E, L> where E: 'a {}

impl<'a, E, L> LabelledArray<'a, E, L> for TpAcArray<'a, E, L>
where
    E: 'a,
{
...
impl<'a, E, L> ArrayRef for TpAcArray<'a, E, L> where E: 'a {}
